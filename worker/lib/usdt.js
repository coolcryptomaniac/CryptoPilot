...
export async function createUsdtIntent(env,user,plan){
  ...
  const c=cfg(env),id=uuid(),units=amountUnits(amount,c.decimals).toString(),db=requireDb(env);
  await db.prepare('INSERT INTO crypto_payments(id,user_id,asset,network,plan,amount,amount_units,target_address,token_address,chain_id,status,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)').bind(id,user.id,'USDT',c.network,plan,amount,units,c.target,c.token,c.chainId,'pending',nowIso()).run();
  await audit(env,user.id,'subscription.usdt_intent',{id,plan,amount,chainId:c.chainId});
  return {intentId:id,asset:'USDT',plan,amount,amountUnits:units,decimals:c.decimals,chainId:c.chainId,network:c.network,tokenAddress:c.token,targetAddress:c.target,minConfirmations:c.confirmations,expiresInMinutes:30};
}
...
export async function verifyUsdtPayment(env,user,{intentId,txHash}={}){
  ...
  await audit(env,user.id,'subscription.usdt_confirmed',{intentId,txHash,plan:row.plan,amount:Number(row.amount),confirmations},'warning');
  return {confirmed:true,plan:row.plan,amount:Number(row.amount),asset:'USDT',txHash,confirmations};
}
